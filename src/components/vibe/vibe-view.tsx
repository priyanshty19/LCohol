...
export function VibeView() {
  ...
  return (
    <div className="space-y-8">
          ...
          {VIBES.map((vibe) => (
            <button
              ...
              className={`relative overflow-hidden rounded-xl border p-4 text-left transition-all duration-200 ${
                selectedVibe === vibe.id
                  ? "border-primary/50 bg-card/80 shadow-md shadow-primary/10"
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <p className="mt-1.5 font-semibold text-sm">{vibe.label}</p>
            ...
            {loading ? (
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
                {Array.from({ length: 4 }).map((_, i) => (
                  <div key={i} className="h-56 animate-pulse rounded-xl bg-card/30" />
                ))}
              </div>
            ) : drinks.length === 0 ? (
          ...
          {cocktailSuggestions.length > 0 && (
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <h2 className="font-semibold">🧪 Cocktails for this vibe</h2>
                <Link
                  href="/mix"
      ...
      {!selectedVibe && (
        <div className="rounded-xl border border-dashed border-border/20 py-16 text-center">
